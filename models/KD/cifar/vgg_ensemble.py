...
class VGG(nn.Module):
    def __init__(self, cfg, batch_norm=False, num_classes=1000):
        ...
        feat_dim = 2048
        self.aux_layer2 = Aux_Layer1(cfg[2][-1], feat_dim, num_classes)
        self.aux_layer3 = Aux_Layer2(cfg[3][-1], feat_dim, num_classes)
    ...
    def forward(self, x):
        ...
        logits = [logits2] + [logits3] + [x]
        features = [features2] + [features3] + [f5]
...
def create_model(model_name, num_classes=100):
    if model_name.lower()=="vgg8_bn":
        model = VGG(cfg["S"], batch_norm=True, num_classes=num_classes)
    elif model_name.lower()=="vgg13_bn":
        model = VGG(cfg["B"], batch_norm=True, num_classes=num_classes)
    return model
