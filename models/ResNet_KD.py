...
class ResNet(nn.Module):
    ...
    def kd_loss(self, logits, logits_mixed, features, features_mixed, y_a, y_b=None, 
                        gamma=0.5, lam=1.0, mixed_loss=True, weights=None, base_weight=2.0,
                        soft_targets=None, weight_of_ce=0.5, tempture=2.0):
        #step 1: feature constrastive learning loss
        batch_size = logits.shape[0]
        logit_scale = self.log_scale.exp()
        features = features / features.norm(dim=1, keepdim=True)
        ...
        #soft_targets = torch.softmax(teacher_logits/tempture, dim=-1)
        
        kd_loss = -torch.sum(soft_targets*soft_probs)/(batch_size*tempture**2)

        if mixed_loss:
            #step 2: knowledge distillation loss
            soft_probs = torch.log_softmax(logits_mixed/tempture, dim=-1)
            kd_loss = -torch.sum(soft_targets*soft_probs, dim=-1)/(tempture**2)
            kd_loss = kd_loss.view(-1, 1)

            #step 3: Cross Entropy loss
            #logpt = (base_weight+modulating_factor)**gamma * F.log_softmax(logits_mixed, dim=1)
            index_a = y_a.view(-1, 1)
            index_b = y_b.view(-1, 1)
            ce_loss = lam*soft_probs.gather(1, index_a).view(-1, 1) + (1-lam)*soft_probs.gather(1, index_b)
            loss = (base_weight+modulating_factor)**gamma * (ce_loss + kd_loss)
            loss = loss.mean()
        else:
            #step 2: knowledge distillation loss
            soft_probs = torch.log_softmax(logits/tempture, dim=-1)
            kd_loss = -torch.sum(soft_targets*soft_probs, dim=-1)/(tempture**2)
            kd_loss = kd_loss.view(-1, 1)
            ...
            #logpt = (base_weight+modulating_factor)**gamma * F.log_softmax(logits_mixed, dim=1)
            index_a = y_a.view(-1, 1)
            ce_loss = soft_probs.gather(1, index_a).view(-1, 1)
            loss = (base_weight+modulating_factor)**gamma * (ce_loss + kd_loss)
            loss = loss.mean()
        #print('CE_Loss=', loss.item(), "   feature_loss=", features_loss.item())
        return loss, weights
