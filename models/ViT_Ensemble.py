...
class VisionTransformer(timm.models.vision_transformer.VisionTransformer):
    ...
    """
    def __init__(self, global_pool=False, selected_layers=[7, 9], normalized=True, aux_depth=2, scale=30, **kwargs):
...
def create_model(vit_type, num_classes=1000, global_pool=False, selected_layers=[7, 9], normalized=True, aux_depth=2, scale=30, **kwargs):
    if vit_type=='ViT_T_16':
        model = VisionTransformer(num_classes=num_classes,
                    patch_size=16, embed_dim=192, depth=12, num_heads=3, mlp_ratio=4, qkv_bias=True,
                    norm_layer=partial(nn.LayerNorm, eps=1e-6), 
                    global_pool=global_pool, selected_layers=selected_layers, normalized=normalized, aux_depth=aux_depth, scale=scale, **kwargs)
    elif vit_type=='ViT_S_16':
        model = VisionTransformer(num_classes=num_classes,
                    patch_size=16, embed_dim=384, depth=12, num_heads=6, mlp_ratio=4, qkv_bias=True,
                    norm_layer=partial(nn.LayerNorm, eps=1e-6),
                    global_pool=global_pool, selected_layers=selected_layers, normalized=normalized, aux_depth=aux_depth, scale=scale, **kwargs)
        
    elif vit_type=='ViT_B_16':
        model = VisionTransformer(num_classes=num_classes,
